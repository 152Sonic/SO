...
int mysystem(char *comando){				//funcao que executa um comando individual
	...
	while(parte!=NULL){
		if(strcmp(parte,"<")==0 || strcmp(parte,">")==0 || strcmp(parte,">>")==0 || strcmp(parte,"2>")==0 || strcmp(parte,"2>>")==0){
			char *tipored;
			tipored=strdup(parte);
			parte=strtok(NULL," ");
			//smyredirect(tipored,parte);
			parte=strtok(NULL," ");
		}else{
			args[i] = parte;
			writemsg(parte);
...
void sigalrm_pipeinativo_handler(int sig){		//envia sinal USR1 ao pai
